...
GO
--update
CREATE TRIGGER [Sales].TrgPhoneNumberChange
ON [Sales].[Customer] FOR UPDATE
AS
IF UPDATE ([StoreID]) OR UPDATE ([TerritoryID])
SELECT 'TEST'
GO
CREATE TRIGGER [Sales].TrgPhoneNumberChange2
ON [Sales].[Customer] FOR UPDATE
AS
IF EXISTS( SELECT [StoreID] FROM inserted) OR EXISTS (SELECT [TerritoryID] FROM inserted)
--IF COLUMNS_UPDATED([StoreID],[TerritoryID]) -- WRONG THIS does not need any arguments
--IF UPDATE ([StoreID]) OR UPDATE ([TerritoryID]) -- GOOD
SELECT 'TEST'
GO
